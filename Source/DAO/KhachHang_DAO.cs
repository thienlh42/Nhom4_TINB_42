...
namespace DAO
{
    public class KhachHang_DAO
    {
        ...
        static public KhachHang_DTO SelectKhachHangLikeTenKH (string tenKH, string diachi, string sdt, string email)
        {
            string sql = "select * from KHACHHANG where ((TenKhachHang like N'%" + tenKH + "%') AND(DiaChi like N'%" +diachi + "%')AND (SDT ='" + sdt + "')AND (Email= '" + email + "'))";
            DataTable dt = DataAccess.ThucThiQuery(sql);
            if (dt.Rows.Count == 0)
            {
                return null;
            ...
            {
                KhachHang_DTO ct = new KhachHang_DTO();
                ct.MaKhachHang = int.Parse(dt.Rows[0].ItemArray[0].ToString());
                return ct;
            }
        }
