...
namespace DAO
{
    public class BaoCaoCongNo_DAO
    {
        ...
        //Cập nhật nợ đầu
        public static string CapNhatNoDauKhachHang(BaoCaoCongNo_DTO r)
        {
            string sql = "update BAOCAOCONGNO set NoDau=" + r.NoDau + " where Thang=" + r.Thang + " and Nam=" + r.Nam + " and MaKhachHang=" + r.MaKhachHang + "";
            return DataAccess.ThucThiNonQuery(sql);
        }
        ...
        public static string XoaBaoCaobyMaKH(int ma)
        {
            string sql = "delete from BAOCAOCONGNO where MaKhachHang=" + ma + "";
            return DataAccess.ThucThiNonQuery(sql);
        }
    }
}
